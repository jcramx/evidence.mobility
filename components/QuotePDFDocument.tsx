...
interface QuotePDFProps {
  ...
  pricingData: {
    ...
    kmToCharge: number;
    minsToCharge: number;
    numericTotal: number; // Subtotal base
    formattedTotal: string;
  };
  taxOptions?: {
    includeTax?: boolean;      // Default: true (Aplica IVA 16%)
    applyRetentions?: boolean; // Default: false (Aplica retenciones si es PM)
    isrRetentionRate?: number; // Default: 0.10 (10%)
    ivaRetentionRate?: number; // Default: 0.106667 (10.66%)
  };
}
...
const styles = StyleSheet.create({
  ...
  // TABLA TARIFARIA Y DESGLOSE FISCAL
  tableContainer: {
    borderWidth: 1,
    borderColor: '#E2E8F0',
    borderRadius: 4,
...
export default function QuotePDFDocument({
  ...
  tripDetails,
  pricingData,
  taxOptions = { includeTax: true, applyRetentions: false }
}: QuotePDFProps) {
  ...
  const includeTax = taxOptions.includeTax ?? true;
  const ivaAmount = includeTax ? subtotal * 0.16 : 0;

  // Retenciones fijas
  const isrRate = taxOptions.isrRetentionRate ?? 0.10; // 10% fijado
  const ivaRate = taxOptions.ivaRetentionRate ?? (2 / 3 * 0.16); // 10.6667% fijado

  const isrRetentionAmount = taxOptions.applyRetentions ? subtotal * isrRate : 0;
  const ivaRetentionAmount = taxOptions.applyRetentions ? subtotal * ivaRate : 0;
  ...
  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {/* Propuesta Económica y Desglose Fiscal */}
        ...
        <View style={styles.tableContainer}>
          ...
          <View style={styles.totalRow}>
            <Text style={styles.totalLabel}>
              {includeTax ? 'TOTAL NETO' : 'TOTAL GARANTIZADO'}
            </Text>
        ...
        <View style={styles.termsBox}>
          ...
          <Text style={styles.termTitle}>Facturación:</Text>
          <Text style={styles.termText}>
            Los precios expuestos reflejan desglose de impuestos fiscales según la normativa vigente. En caso de requerir factura CFDI, favor de adjuntar su Constancia de Situación Fiscal actualizada.
          </Text>
        </View>
